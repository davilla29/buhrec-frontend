...
const ProposalDecision = () => {
  ...
  const getHeaderConfig = () => {
    if (isApproved)
      return { text: "Your proposal was approved", color: "text-[#003B95]" };
    if (isRejected)
      return { text: "Your proposal was rejected", color: "text-red-700" };
    if (isChangesRequested)
      return { text: "Changes were requested", color: "text-amber-600" };
    return { text: "Review not yet completed", color: "text-gray-500" };
  };
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 font-sans text-center bg-white">
      <div className="w-full max-w-4xl">
        {/* HEADER */}
        <div className="mb-10 mt-12 px-4 text-left">
          <h2 className={`text-xl font-bold mb-2 ${header.color}`}>
            {header.text}
          </h2>
          <h1 className="text-3xl font-bold text-gray-900 mb-2">
            {data.title}
        ...
        {/* FEEDBACK SECTION (For Rejected or Changes Requested) */}
        {(isApproved || isRejected || isChangesRequested) && data.decisionReason && (
        ...
        {isPending && (
          <div className="mb-20 px-4">
            <p className="text-gray-400 italic">
              Your proposal is currently under review. Please check back later
        ...
        {isApproved && (
          <>
            <div className="mb-10 scale-50 sm:scale-75 md:scale-90 origin-top flex justify-center">
              <ApprovalCertificate proposal={data} />
            </div>
        ...
        <div className="flex gap-4 justify-center pb-12">
          <button
            onClick={() => navigate("/researcher/dashboard/my-proposals")}
            className={`px-8 py-3 rounded-full cursor-pointer text-white transition-colors text-sm font-medium ${
              isApproved
                ? "bg-gray-600 hover:bg-gray-700"
                : isRejected
                  ? "bg-red-700 hover:bg-red-800"
          ...
          >
            {isPending ? "Back to dashboard" : "Back to proposals"}
          </button>
          ...
          {isApproved && (
            <button
              onClick={handleDownload}
              disabled={downloading}
              className="px-10 py-3 cursor-pointer rounded-full bg-[#003B95] text-white text-sm font-medium flex items-center gap-2"
            >
              {downloading ? (
                <>
                  <Loader2 className="w-4 h-4 animate-spin" />
